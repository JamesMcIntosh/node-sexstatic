...
var sexstatic = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next, mimeType) {
    ...
    // TODO: extract SEO path and rewriting into config
    const SEO_PATH = /\/static-files\/(.*?)\/(.*?)\/(.*)/g;
    function seoUrl() {
      // see if this is a SEO url
        const staticFilesIdx = parsed.pathname.indexOf('/static-files/');
        const fileNameStartIdx = staticFilesIdx + '/static-files/'.length;
        const fileNameEndIndex = parsed.pathname.indexOf('/', fileNameStartIdx);
        const mimeTypeEndIndex = parsed.pathname.indexOf('/', fileNameEndIndex + 1);
        const nonSeoPath = parsed.pathname.substring(0, fileNameEndIndex);
        const nonSeoUrl = nonSeoPath + ((parsed.search) ? parsed.search : '');
        const mimeType = decodeURIComponent(parsed.pathname.substring(fileNameEndIndex + 1, mimeTypeEndIndex));
        
        log(`SEO url: ${nonSeoUrl}, mimeType: ${mimeType}`);
    ...
    function onFileNotFound() {
      ...
      log(`PATHNAME: ${parsed.pathname}`);

      if (req.statusCode == 404) {
        // This means we're already trying ./404.html
        status[404](res, next);
      } else if (parsed.pathname.match(SEO_PATH)) {
        seoUrl();
      }
    ...
    function onDirectoryFound(stat) {
      if (defaultExt && !path.extname(parsed.pathname).length) {
        var defFilePath = path.normalize(
          path.join(root,
            path.relative(
              path.join('/', baseDir),
              pathname.endsWith('/') ? pathname.substring(0, pathname.length - 1) : pathname + '.' + defaultExt
            )
          )
        )
        log('FILE: ' + defFilePath);
        var exists = fs.existsSync(defFilePath);
        
        if (exists) {
          var defPath = parsed.pathname;
          if (defPath.endsWith('/')) {
            defPath = defPath.substring(0, defPath.length - 1);
          }
          log('TRY: ' + defPath + '.' + defaultExt);

          middleware({
            url: defPath + '.' + defaultExt + ((parsed.search)? parsed.search:'')
          }, res, next);
    ...
    function serve(stat) {
      ...
      // special case.
      var contentType = mimeType ? mimeType : mime.lookup(file), charSet;
...
if (!module.parent) {
  ...
  http.createServer(sexstatic(dir, opts))
    .listen(port, function () {
      log('sexstatic serving ' + dir + ' at http://0.0.0.0:' + port);
    });
}
